...
class TestAuthToken(base.BaseCLITestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.parser.add_argument('--apikey', dest='apikey')
    ...
    def test_decorate_api_key_by_cli(self):
        token = uuid.uuid4().hex
        args = self.parser.parse_args(args=['--apikey', token])
        self.assertDictEqual(self._mock_run(args), {'apikey': token})
    ...
    def test_decorate_api_key_to_http_headers(self):
        token = uuid.uuid4().hex
        kwargs = self._mock_http('/', apikey=token)
